    ...
    with open(publish_file, 'r') as file:
        ...
        for row in reader:
            if len(row) == 4:
                ...
                if rc == 0:
                    print(f"设备属性上报成功:{rc}, 请求ID:{request_id}")
                else:
                    print(f"设备属性上报失败, rc={rc}")
                count += 1
                print(f"已发布 {count} 条记录。")
                time.sleep(1)  # 防止发送过快，根据平台限制可能需要调整
        print("数据发布完成。已发布记录总数:", count)
...
while True:
    ...
    if not connected:
        ...
        if msg == "1":
            ...
            if rc == 0:
                print("订阅成功, 主题=%s" % mqtt_topic_post)
            else:
                print("订阅失败, rc=%d" % rc)
        elif msg == "4": # 输入为4时，取消订阅post这个主题
            rc, mid = lk.unsubscribe_topic(mqtt_topic_post)
            if rc == 0:
                print("取消订阅成功, 主题=%s" % mqtt_topic_post)
            else:
                print("取消订阅失败, rc=%d" % rc)
            ...
            rc, mid = lk.unsubscribe_topic([mqtt_topic_post,
                                            mqtt_topic_set])
            if rc == 0:
                print("取消订阅多个主题成功:%r, mid:%r" % (rc, mid))
            else:
                print("取消订阅失败, rc=%d" % rc)
        elif msg == "8": # RRPC请求
            lk.on_message = on_message # 收到消息后的回调
            ...
            if rc == 0:
                print("设备属性上报成功:%r, 请求ID:%r" % (rc, request_id))
            else:
                print("设备属性上报失败, rc=%d" % rc)
        elif msg == "12": # 属性上报
            # 上报随机属性
            prop_data = {
